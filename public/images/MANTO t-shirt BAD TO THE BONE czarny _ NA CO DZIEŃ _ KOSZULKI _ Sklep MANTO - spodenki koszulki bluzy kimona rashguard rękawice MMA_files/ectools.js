// (c) by Gemius SA - gemius ecommerce tools
// ver. 1.3

var gemius_extraparameters = gemius_extraparameters || [];
var gemius_ecommerce_identifier = gemius_ecommerce_identifier || '';

function gemius_pending(i) { window[i] = window[i] || function() {var x = window[i+'_pdata'] = window[i+'_pdata'] || []; x[x.length]=arguments;};};
gemius_pending('gemius_hit'); gemius_pending('gemius_event'); gemius_pending('pp_gemius_hit'); gemius_pending('pp_gemius_event');

if (typeof gemius_ectools == 'undefined') {
	gemius_ectools = function(ectype,ecdata) {
		if (typeof ectype != 'undefined') {
			var mkeys = {
				'category':'gP_ICN','category_id':'gP_IC','currency':'gP_CC','discount':'gP_IUD',
				'discount_id':'gP_IDI','EAN':'gP_IGI','id':'gP_I','language':'gP_LG',
				'name':'gP_IN','order':'gP_O','page':'gP_PT','price':'gP_IUP','quantity':'gP_IQ',
				'payment':'gP_OPT','producer':'gP_IP','shipping':'gP_OST','shipping_value':'gP_OSV',
				'tax':'gP_IUT','total_discount':'gP_ODV','total_discount_id':'gP_ODI',
				'total_tax':'gP_OT','value':'gP_OV','variation':'gP_IV'
			};
			var validkeys = [];
			var evtype = '';
			if (ectype=='additem') {
				validkeys = ['id','name','category','category_id','price','quantity','tax','discount','discount_id','producer','EAN','variation','currency','language'];
				evtype = '_action_';
			} else if (ectype=='item') {
				validkeys = ['order','id','name','category','category_id','price','quantity','tax','discount','discount_id','producer','EAN','variation','currency','language'];
				evtype = '_order_';
			} else if (ectype=='order') {
				validkeys = ['order','value','total_tax','shipping_value','shipping','payment','total_discount_id','total_discount','currency','language'];
				evtype = '_order_';
			} else if (ectype=='view') {
				validkeys = ['page','id','name','category','category_id','price','tax','discount','discount_id','producer','EAN','variation','currency','language'];
				evtype = '*';
			} else if (ectype=='viewcategory') {
				validkeys = ['category','category_id','language'];
				evtype = '*';
			} else if (ectype=='viewitem') {
				validkeys = ['id','name','category','category_id','price','tax','discount','discount_id','producer','EAN','variation','currency','language'];
				evtype = '*';
			}
			if (evtype!='') {
				var evargs = [];
				if (typeof ecdata != 'undefined') {
					for (var key in ecdata) {
						for (var vki in validkeys) {
							var mkey = validkeys[vki];
							if (key==mkey && typeof mkeys[mkey] != 'undefined') {
								if (typeof ecdata[key].substring != 'undefined') {
									evargs[evargs.length]=mkeys[mkey]+'='+ecdata[key].substring(0,899);
								} else {
									evargs[evargs.length]=mkeys[mkey]+'='+ecdata[key];
								}
							}
						}
					}
				...
				if (evtype!='*') {
					gemius_event.apply(this,[evtype,gemius_ecommerce_identifier].concat(evargs));
				} else {
					if (gemius_ecommerce_identifier=='') {
						gemius_extraparameters = gemius_extraparameters.concat(evargs);
					} else {
						gemius_event.apply(this,['_view_',gemius_ecommerce_identifier].concat(evargs,gemius_extraparameters));
					}
				}
			}
		}
	}
	if (typeof window.gemius_ecommerce_pdata != 'undefined') {
		var i;
		for (i=0 ; i<window.gemius_ecommerce_pdata.length ; i++) {
			gemius_ectools.apply(this,window.gemius_ecommerce_pdata[i]);
		}
	}
	gemius_ecommerce = gemius_ectools;
}
...
if (typeof gemius_hcconn == "undefined") {
	gemius_hcconn = {
		...
		hc : (typeof gemius_hitcollector != 'undefined')?gemius_hitcollector:(typeof pp_gemius_hitcollector != 'undefined')?pp_gemius_hitcollector:'smpl.hit.gemius.pl',
